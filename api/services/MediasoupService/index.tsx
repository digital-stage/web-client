...
const MediasoupService = () => {
  ...
  React.useEffect(() => {
    if(connection?.routerConnection && connection.receiveTransport) {
      ...
      setVideoConsumers(prevState => {
        ...
        videoTracks.map(track => {
          if (!prevState[track._id]) {
            consume(connection.routerConnection, connection.receiveTransport, connection.device, track.producerId)
    ...
    if (
      ...
      return () => {
        ...
        publishedIds.map(publishedId => {
          unpublishProducer(emit, publishedId, "video").catch((err) =>
            reportError(err)
          )
        })
      }
