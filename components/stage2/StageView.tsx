import {
    clientActions, useEmit, useRemoteVideoTracks,
...
const Box = ({
    ...
    const numCols = useStageSelector<number>(state => state.globals.localDeviceId && state.devices.byId[state.globals.localDeviceId].numCols || 3)
                    ...
                    <div className="inner">
                        ...
                        <div className={`info ${!track ? 'centered' : ''}`}>
                            ...
                            <div className="names">
                                ...
                                {videoMuted && <span style={{fontSize: "0.6rem", color: "var(---danger)"}}>Schlechte Internetverbindung</span>}
                            </div>
                        </div>
                    </div>
                </div>
...
const StageMemberView = ({stageMemberId, color, groupName}: { stageMemberId: string, color: string, groupName?: string }): JSX.Element => {
...
const GroupView = ({groupId, showName}: { groupId: string, showName?: boolean }): JSX.Element | null => {
        ...
        return (
            <>
                <div className="group" style={{
                    borderColor: color,
                    overflowX: showLanes ? 'visible' : undefined,
                    height: showLanes ? '100%' : undefined,
                    flexWrap: showLanes ? 'nowrap' : 'wrap',
                    flexGrow: showLanes ? 1 : 0
                }}>
...
const StageView = (): JSX.Element => {
    ...
    return (
        <>
            <div className="stageGrid" style={{
                width: showLanes ? 'auto' : '100%',
                flexDirection: showLanes ? 'row' : 'column',
                overflowX: showLanes ? 'scroll' : undefined,
            }}
            >
                {groupIds.map(groupId => <GroupView key={groupId} groupId={groupId} showName={groupIds.length > 1}/>)}
