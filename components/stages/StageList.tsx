...
const StageMemberRow = (props: { id: string; stage: Stage }) => {
            ...
            {isAdmin && (
                <>
                    <label>
                        <input
                            type="checkbox"
                            checked={isSoundEditor}
                            onChange={() => {
                                if (isSoundEditor) {
                                    connection.emit(ClientDeviceEvents.ChangeStage, {
                                        _id: stage._id,
                                        soundEditors: stage.soundEditors.filter(
                                            (editor) => editor !== user._id
                                        ),
                                    })
                                } else {
                                    connection.emit(ClientDeviceEvents.ChangeStage, {
                    ...
                    <DangerButton
                        onClick={() => connection.emit(ClientDeviceEvents.RemoveStageMember, id)}
                    >
                        Kick
                    </DangerButton>
                </>
...
const GroupRow = (props: { id: string; stage: Stage }) => {
    const { id, stage } = props
        ...
        <div>
            ...
            <div>
                {groupIds.map((groupId) => (
                    <GroupRow key={groupId} id={groupId} stage={stage} />
                ))}
            </div>
        </div>
    )
