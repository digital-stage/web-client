...
import {Group, useStageSelector} from "@digitalstage/api-client-react";
...
const MixingPanel = (): JSX.Element => {
  ...
  const groups = useStageSelector<Group[]>((state) => {
    if (state.globals.stageId && state.groups.byStage[state.global.stageId]) {
      return state.groups.byStage[state.globals.stageId].map((id) => state.groups.byId[id]);
    }
    return [];
  });
